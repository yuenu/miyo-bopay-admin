...
const Paid = ({ visible, data, onCancel, onOk, loading }) => {
  const { offlineGateways } = useSelector(selectTransfer);
  ...
  useEffect(() => {
    if (visible) {
      const obj = offlineGateways.find(i => i.id === data.gateway_id);
      form.setFieldsValue({
        ...data,
    ...
    >
      <Spin spinning={loading}>
        <Form {...formLayout} form={form}>
          ...
          >
            <SearchSelect
              action={getTransfersOfflineGateways}
              params={{ id: data.id }}
              payloadPos="data"
